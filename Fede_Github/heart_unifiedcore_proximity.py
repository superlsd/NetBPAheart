...
with open('input/Unifiedcoreheart.txt') as f:
...
heart_unifiedcore_proximity={}

bpa_spec_size=len(Unifiedcoreheart_list)
bpa_random_distance={}
...
for dis in disgenet_gda_curated_score_multigenes.keys():
    heart_unifiedcore_proximity["Unifiedcore",dis]=calculate_proximity(G_ppi,spl,Unifiedcoreheart_list,disgenet_gda_curated_score_multigenes[dis],bpa_random_distance)


#Let's save it
with open('output/heart_unifiedcore_proximity.pickle', 'wb') as handle:
    pk.dump(heart_unifiedcore_proximity, handle, protocol=pk.HIGHEST_PROTOCOL)
